...
def diagnose(records, targets, interval, lang="zh"):
    ...
    if len(confirmed) >= 3:
        ...
        if pattern == "continuous":
            if en:
                verdict = ("Likely past your modem — worth a call to your ISP",
                           ...
                           "the line dropping and reconnecting repeatedly, but ping data alone only shows the "
                           "internet segment is the problem, not the exact cause — that's easiest to confirm on "
                           "the ISP's side. The simplest next step is to contact your ISP's support with this "
                           "report and describe the pattern (lags a few seconds to tens of seconds every few "
                           "minutes) and ask them to check their side for repeated disconnects — far less "
                           "hassle than digging through your modem's admin page yourself. (If you're comfortable "
            ...
            if en:
                verdict = ("Likely past your modem — worth a call to your ISP",
                           ...
                           f"with no fixed pattern yet. Confidence: {confidence}. If the lag clusters around "
                           "downloads/streaming, it may instead be router QoS worth adjusting. Otherwise, the "
                           "simplest next step is contacting your ISP's support with this report and asking "
                           "them to check their side for disconnects — simpler than digging through your "
